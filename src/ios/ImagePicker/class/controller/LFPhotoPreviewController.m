...
//#endif
{
    ...
    UILabel *_videoMaxTimeLabel; //视频最大发送时长提示
...
- (void)configCustomNaviBar {
    ...
    if (self.isPhotoPreview) {
        ...
        _backButton.imageEdgeInsets = UIEdgeInsetsMake(0, image.size.width-50+8+10, 0, 0);
    }
...
//#endif
    ...
    if (imagePickerVc.allowPickingType == LFPickingMediaTypeVideo) {
        _videoMaxTimeLabel = [[UILabel alloc] init];
        CGFloat labelW = 300;
        _videoMaxTimeLabel.frame = CGRectMake((CGRectGetWidth(_toolSubBar.frame)-labelW)/2, 0, labelW, CGRectGetHeight(_toolSubBar.frame));
        _videoMaxTimeLabel.autoresizingMask = UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleBottomMargin;
        _videoMaxTimeLabel.textAlignment = NSTextAlignmentCenter;
        _videoMaxTimeLabel.font = toolbarTitleFont;
        _videoMaxTimeLabel.textColor = toolbarTitleColorNormal;
        _videoMaxTimeLabel.alpha = 0.6;
        [toolSubBar addSubview:_videoMaxTimeLabel];
    }
...
//    _editButton.enabled = false;
    if (self.models.count > self.currentIndex) {
        ...
        if (model.type == LFAssetMediaTypePhoto) {
            ...
            videoEditingVC.maxClippingDuration = imagePickerVc.maxVideoDuration;
            //编辑视图导航栏
            videoEditingVC.editNaviBgColor = imagePickerVc.editNaviBgColor;
            videoEditingVC.editOKButtonTitleColorNormal = imagePickerVc.editOKButtonTitleColorNormal;
            videoEditingVC.editCancelButtonTitleColorNormal = imagePickerVc.editCancelButtonTitleColorNormal;
            //编辑视图底部工具栏
            videoEditingVC.editToolbarBgColor = imagePickerVc.editToolbarBgColor;
            videoEditingVC.editToolbarTitleColorNormal = imagePickerVc.editToolbarTitleColorNormal;
            videoEditingVC.editToolbarTitleColorDisabled = imagePickerVc.editToolbarTitleColorDisabled;
