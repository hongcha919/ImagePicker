...
@interface LFPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,CustomPhotoEditingControllerDelegate>
{
...
- (void)configBottomToolBar {
    ...
    if (imagePickerVc.allowPreview) {
        ...
        if (imagePickerVc.maxImagesCount <= 1 && imagePickerVc.allowPickingType == LFPickingMediaTypePhoto) {
            _previewButton.enabled = NO;
            _previewButton.alpha = 0.5;
        }
...
//#endif
                ...
                dispatch_group_notify(_group, dispatch_get_main_queue(), ^{
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.25f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        [imagePickerVc hideProgressHUD];
                        //只有单独选择图片或者视频时支持云端上传功能
                        if (imagePickerVc.allowPickingType == LFPickingMediaTypePhoto || imagePickerVc.allowPickingType == LFPickingMediaTypeVideo) {
                            NSLog(@"进入    跳转上传页面");
                            if (imagePickerVc.isGetCloudPath) {
                                NSLog(@"跳转上传页面");
                                //跳转上传页面
                                ...
                                [uploadVC setDoneButtonClickBlock:^(NSMutableArray * _Nonnull resultArray) {
                                    ...
                                    if (imagePickerVc.autoDismiss) {
                                        [imagePickerVc dismissViewControllerAnimated:YES completion:^{
                                            [weakSelf callDelegateMethodWithCloudResults:resultArray];
                                        }];
                                    } else {
                                        [weakSelf callDelegateMethodWithCloudResults:resultArray];
                                    }
...
- (void)callDelegateMethodWithResults:(NSArray <LFResultObject *>*)results {
    ...
    } else if ([pickerDelegate respondsToSelector:@selector(lf_imagePickerController:didFinishPickingResult:isOriginal:)]) {
        [pickerDelegate lf_imagePickerController:imagePickerVc didFinishPickingResult:results isOriginal:imagePickerVc.isSelectOriginalPhoto];
    }
...
/**
 ...
 @param results 云端图片视频数据结果
 */
- (void)callDelegateMethodWithCloudResults:(NSArray*)results {
    
    LFImagePickerController *imagePickerVc = (LFImagePickerController *)self.navigationController;
    id <LFImagePickerControllerDelegate> pickerDelegate = (id <LFImagePickerControllerDelegate>)imagePickerVc.pickerDelegate;
    
    if ([pickerDelegate respondsToSelector:@selector(lf_imagePickerController:didFinishPickingCloudResult:)]) {
        [pickerDelegate lf_imagePickerController:imagePickerVc didFinishPickingCloudResult:results];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    __weak __typeof__(self) weakSelf = self;
    __weak __typeof__(imagePickerVc) weakImagePickerVc = imagePickerVc;
...
//    }
    ...
    if (imagePickerVc.maxImagesCount == 1 && model.type == LFAssetMediaTypePhoto /*&& imagePickerVc.cutType <2*/) {
        cell.onlyClick = YES;
    } else {
        cell.onlyClick = NO;
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (/*imagePickerVc.cutType<=1 && */imagePickerVc.maxImagesCount==1 && imagePickerVc.allowPickingType == LFPickingMediaTypePhoto) {
        ...
        if (model.type == LFAssetMediaTypePhoto && imagePickerVc.allowEditing) {
            ...
            photoEditingVC.allowEditing = imagePickerVc.allowEditing;
            
            if (imagePickerVc.editOKButtonTitle) {
                photoEditingVC.oKButtonTitle = imagePickerVc.editOKButtonTitle;
            }
            if (imagePickerVc.editCancelButtonTitle) {
                photoEditingVC.cancelButtonTitle = imagePickerVc.editCancelButtonTitle;
            }
            if (imagePickerVc.editOKButtonTitleColorNormal) {
                photoEditingVC.oKButtonTitleColorNormal = imagePickerVc.editOKButtonTitleColorNormal;
            }
            if (imagePickerVc.editCancelButtonTitleColorNormal) {
                photoEditingVC.cancelButtonTitleColorNormal = imagePickerVc.editCancelButtonTitleColorNormal;
            }
...
{
    LFImagePickerController *imagePickerVc = (LFImagePickerController *)self.navigationController;
    LFAsset *model = imagePickerVc.selectedModels[0];
    /** 缓存对象 */
...
- (void)pushEditViewController:(CustomPhotoEditingController *)photoEditVc
{
    photoEditVc.isHiddenNavBar = NO;
    photoEditVc.isHiddenStatusBar = NO;
