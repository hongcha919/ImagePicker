...
+ (NSString *)lf_MediaPickerUploadBundlePath {
    static NSString *lfMediaPickerUploadBundlePath = nil;
    if (lfMediaPickerUploadBundlePath == nil) {
        // 这里不使用mainBundle是为了适配pod 1.x和0.x
        lfMediaPickerUploadBundlePath = [[NSBundle mainBundle] pathForResource:@"MediaPickerUpload" ofType:@"bundle"];
    }
    return lfMediaPickerUploadBundlePath;
}

+ (UIImage *)lf_MediaPickerUploadImage:(NSString *)name
{
    NSString *imgPath = [[self lf_MediaPickerUploadBundlePath] stringByAppendingPathComponent:name];
