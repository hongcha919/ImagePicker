...
- (CGPathRef)drawGrid
{
    ...
    self.lineDashPattern = self.lineDashPatternAry;
    ...
    if (self.cutType==0) {
        //圆形
        path = [UIBezierPath bezierPathWithOvalInRect: CGRectMake(rct.origin.x, rct.origin.y, rct.size.width, rct.size.height)];
    } else if (self.cutType==1) {
        //矩形
        path = [UIBezierPath bezierPathWithRect: CGRectMake(rct.origin.x, rct.origin.y, rct.size.width, rct.size.height)];
    } else {
        //九宫格
